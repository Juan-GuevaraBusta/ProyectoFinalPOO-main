...
Escenario::Escenario() : fondoCargado(false) {
    ...
    agregarPlataforma(0.0f, 485.0f, 300.0f, 35.0f); // Plataforma 2
}
...
float Escenario::getAlturaPlatformaEn(float posX, float posY) {
    ...
    for (const auto& plataforma : plataformas) {
        ...
        if ( ( posX >= bounds.left && posX < bounds.left + bounds.width ) && (posY >= bounds.top && posY < bounds.top + bounds.height) ) {
